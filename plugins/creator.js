...
let name = await conn.getName(who)
 ...
 const ini = await conn.sendContactArray(m.chat, [
    [`${nomorown}`, `${await conn.getName(nomorown+'@s.whatsapp.net')}`, `👑 Developer Bot `, `🚫 Don't call me 🥺`, `⁨meguminbot12@gmail.com`, `🇮🇩 Indonesia`, `🚀 https://instagram.com/mhmdfjralfarizi_`, `Owner Baik Hati Kok! `],
    [`${conn.user.jid.split('@')[0]}`, `${await conn.getName(conn.user.jid)}`, `🔥 Bot WhatsApp 🐣`, `📵 Don't spam/call me 😢`, `Nothing`, `🇮🇩 Indonesia`, `🚀 https://instagram.com/mhmdfjralfarizi`, `🤖 Dont Forget To Donate!`]
  ], fkontak)
